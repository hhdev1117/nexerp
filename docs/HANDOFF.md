...
`codex/enterprise-access`에 관리자 전용 전사 권한관리 화면, 정책 판정 엔진, 회사별 정책 저장·감사 RPC를 추가했다. 정책 발행·복원, 서버 기반 회사 전환·메뉴 접근, 회사·사업장 RLS까지 연결했다. 최초 발행 전에는 기존 권한을 유지하며 최초 발행부터 일반 사용자의 회사별 정책 연결을 강제한다. 상세 상태와 미구현 후속 범위는 [전사 권한 구현 상태](setup/enterprise-access.md)를 참조한다. 신규 `20260914000200`, `20260914000300` 마이그레이션은 이 작업에서 운영 적용하지 않았다. 추가로 `20260914000400` 직원 원장·날짜별 발령·현재 권한 자동 연계와 `/hr/employees` 화면을 구현했다. 004도 운영 미적용이다. [직원 명부 안내](setup/hr-ledger.md)에 설정·제약을 기록했다. 005 조직·직급·직책 기준정보와 직원 이름·입사일 정정·감사 이력을 추가했다. [기준정보·정정 안내](setup/hr-reference.md)에 적용 순서와 제한을 기록했으며 005 역시 운영 미적용이다. 006 회사별 인사 기본 모듈의 사용·진행 건 정리·읽기 전용·중지와 메뉴 표시 설정을 추가했다. 007은 기존 직원의 로그인 계정 연결·교체·해제, 권한 레벨 미리보기와 감사 이력을 추가했다. [모듈 설정 안내](setup/hr-modules.md)와 [계정 연결 안내](setup/hr-account-links.md)를 참조한다. 전체 테스트 783개, 신규 계정 연결 PostgreSQL 검증 38개, ESLint·빌드와 데스크톱·모바일 확인을 통과했다. 006·007 모두 운영 미적용이다. 아래 기록은 기존 main의 운영 기준이다.
