...
`codex/enterprise-access`에 관리자 전용 전사 권한관리 화면, 정책 판정 엔진, 회사별 정책 저장·감사 RPC를 추가했다. 레벨·직급/직책·사용자 역할·예외는 전환 준비 설정이며 기존 업무 권한은 유지한다. 상세 상태와 미구현 후속 범위는 [전사 권한 구현 상태](setup/enterprise-access.md)를 참조한다. 신규 `20260914000200` 마이그레이션은 이 작업에서 운영 적용하지 않았다. 아래 기록은 기존 main의 운영 기준이다.
