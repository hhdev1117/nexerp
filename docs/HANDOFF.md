...
### 겸직·부서 범위 권한 (로컬 구현 완료, 운영 미적용)

- 마이그레이션: `20260915000900_hr_secondary_assignments.sql`
- 화면: 직원 상세의 `EmployeeSecondaryAssignments.vue`
- 규칙: 주 소속 개인 직급 공통 사용, 겸직은 부서·사업장·직책만 지정, 동일 부서 기간 중복 금지
- 서버 권한: `private.enterprise_granted_for_target`으로 현재 부서 계층과 사업장 범위를 검사하고 개인 거부를 우선 적용
- 운영 적용 전 [겸직과 부서 범위 권한 안내](setup/hr-secondary-assignments.md)의 마이그레이션·RLS 검증 절차를 수행할 것
