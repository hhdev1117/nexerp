...
`codex/enterprise-access`에 관리자 전용 전사 권한관리 화면, 정책 판정 엔진, 회사별 정책 저장·감사 RPC를 추가했다. 정책 발행·복원, 서버 기반 회사 전환·메뉴 접근, 회사·사업장 RLS까지 연결했다. 최초 발행 전에는 기존 권한을 유지하며 최초 발행부터 일반 사용자의 회사별 정책 연결을 강제한다. 상세 상태와 미구현 후속 범위는 [전사 권한 구현 상태](setup/enterprise-access.md)를 참조한다. 신규 `20260914000200`, `20260914000300` 마이그레이션은 이 작업에서 운영 적용하지 않았다. 아래 기록은 기존 main의 운영 기준이다.
