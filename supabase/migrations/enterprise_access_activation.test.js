...
describe('enterprise activation SQL contract',()=>{
 it('exposes own JSON context and administrator publication RPCs only',()=>{
  for(const name of ['enterprise_access_context','enterprise_access_publication','enterprise_publish_access_policy','enterprise_revert_access_policy','enterprise_access_companies','enterprise_access_sites']) expect(sql).toContain('function public.'+name);
  for(const key of ['mode','companyId','companies','menuKeys','revision','companyActions','siteActions']) expect(sql).toContain("'"+key+"'");
  expect(sql).toContain('auth.uid()');expect(sql).toContain("'Asia/Seoul'");
 ...
 it('publishes locked saved revisions and only restores an existing snapshot',()=>{
  expect(sql).toContain('for update');expect(sql).toContain('draft_revision_conflict');expect(sql).toContain('no_previous_publication');expect(sql).toContain('no_active_membership');
 ...
 it('closes unactivated-company fallback after activation and checks separate navigation actions',()=>{
  ...
  expect(sql).toContain('not global_active');expect(sql).toContain("p->>'effect'='deny'");
 });
});
