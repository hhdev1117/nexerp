...
await admin();await rejects(()=>directory(),'42501');
...
await save(hr,0);await publish(1,0);
await rejects(()=>directory(),'42501');
await login(actor);eq((await directory()).employees,[]);
eq((await directory()).accounts,[{id:'61000000-0000-0000-0000-000000000003',name:'Future'},{id:actor,name:'User'}]);
...
await login(actor);eq((await directory()).accounts,[{id:actor,name:'User'}]);
await rejects(()=>create({...employee,profileId:'61000000-0000-0000-0000-000000000003'}),'22023');
await db.exec("reset role;update profiles set is_active=true where id='61000000-0000-0000-0000-000000000003'");await login(actor);
...
await admin();
const scoped=structuredClone(hr);scoped.levels[0].permissions.filter(p=>p.resource==='hr.core').forEach(p=>p.scope='site');
await save(scoped,1);await publish(2,1);await login(actor);
await rejects(()=>directory(),'42501');eq((await context()).menuKeys.includes('hr.core'),false);
await admin();await save(hr,2);await publish(3,2);await login(actor);
...
const noCreate=structuredClone(snapshot);noCreate.levels[0].permissions=noCreate.levels[0].permissions.filter(p=>!(p.resource==='hr.core'&&p.action==='create'));
...
await login(actor);eq((await directory()).accounts,[]);eq((await directory()).permissions.create,false);
await db.exec('reset role');await db.query('update enterprise_access_publications set policy=$2 where company_id=$1 and revision=3',[company,JSON.stringify(snapshot)]);await login(actor);
...
eq(derived.organizationId,'D2');eq(derived.grade,'G2');eq(derived.level,1);
// A narrower deny cannot leak a company-wide directory.
await admin();const narrow=structuredClone(hr);narrow.overrides=[{actorId:actor,resource:'hr.core',action:'read',scope:'site',effect:'deny',from:null,to:null}];
await save(narrow,3);await publish(4,3);await login(actor);await rejects(()=>directory(),'42501');eq((await context()).menuKeys.includes('hr.core'),false);
await admin();await rpc('enterprise_revert_access_policy',[company,4],"$1,$2,'restore'");await login(actor);
// Simulate the next Seoul day by moving this fixture's effective date to yesterday.
await db.exec('reset role');await db.query('update hr_personnel_actions set effective_date=effective_date-1 where employee_id=$1 and cancelled_at is null',[id]);await login(actor);
// Membership required: a linked profile is not automatically authorized.
await login('61000000-0000-0000-0000-000000000003');await rejects(()=>directory(),'42501');await login(actor);
await db.exec('reset role');await db.query('update sites set is_active=false where id=$1',[site]);await login(actor);
await record(action('terminate',today),6);
eq((await context()).companyId,null);await rejects(()=>directory(),'42501');await db.exec('reset role');eq((await db.query('select private.hr_employee_state($1,$2) s',[company,id])).rows[0].s.department,'D2');await login(actor);
// Publication reversal must not restore a terminated employee.
await admin();await save(hr,4);await publish(5,5);await rpc('enterprise_revert_access_policy',[company,6],'$1,$2,\'restore\''.replaceAll('\\',''));
await login(actor);eq((await context()).companyId,null);
...
mapped.mappings=[{kind:'grade',code:'G',level:1,from:null,to:null},{kind:'position',code:'P2',level:4,from:null,to:null}];
...
await admin();await saveOther(mapped,0);await publishOther(1,0);await login(actor);
...
await admin();mapped.members[0].level=1;await saveOther(mapped,1);await publishOther(2,1);await login(actor);
eq((await context(other)).menuKeys.includes('sales.orders'),false);
eq((await context(other)).menuKeys.includes('hr.core'),true);
console.log(`HR migration: ${checks} PostgreSQL runtime assertions passed.`);await db.close();
