...
const admin = '71000000-0000-0000-0000-000000000001',
    hr = '71000000-0000-0000-0000-000000000002';
const staff = '71000000-0000-0000-0000-000000000003',
    lead = '71000000-0000-0000-0000-000000000004';
const spare = '71000000-0000-0000-0000-000000000005',
    dormant = '71000000-0000-0000-0000-000000000006';
...
for (const file of fs
    ...
    .filter((f) => f.endsWith('.sql') && f < '20260914000700')
    .sort())
...
await db.query('insert into auth.users(id,email,raw_app_meta_data) select unnest($1::uuid[]),unnest($2::text[]),$3::jsonb', [
    [admin, hr, staff, lead, spare, dormant],
    ['admin@gmail.com', 'hr@gmail.com', 'staff@gmail.com', 'lead@gmail.com', 'spare@gmail.com', 'dormant@gmail.com'],
    provisioned
]);
...
const permissions = ['menu', 'read', 'create', 'update']
    .map((action) => ({ resource: 'hr.core', action, scope: 'company' }))
    .concat([
        { resource: 'dashboard', action: 'menu', scope: 'company' },
        { resource: 'dashboard', action: 'read', scope: 'company' }
    ]);
const policy = {
...
eq(
    (await options(second)).candidates.map((c) => c.name),
    ['HR담당', '사원계정', '예비계정', '팀장계정']
);
...
eq(view.revision, 2);
eq(
    (await history()).map((row) => [row.beforeAccountId, row.afterAccountId, row.reason]),
    [[null, staff, '계정 연결 변경']]
);
eq(
    (await options(second)).candidates.map((c) => c.id),
    [hr, spare, lead]
);
...
eq(
    (await history()).map((row) => [row.beforeAccountId, row.afterAccountId]),
    [
        [staff, spare],
        [null, staff]
    ]
