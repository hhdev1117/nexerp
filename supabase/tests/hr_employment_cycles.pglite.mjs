...
const admin = '81000000-0000-0000-0000-000000000001', hr = '81000000-0000-0000-0000-000000000002';
const oldAccount = '81000000-0000-0000-0000-000000000003', newAccount = '81000000-0000-0000-0000-000000000004';
const spare = '81000000-0000-0000-0000-000000000005', company = '82000000-0000-0000-0000-000000000001';
...
for (const file of fs.readdirSync('supabase/migrations').filter(f => f.endsWith('.sql') && f < '20260915000800').sort()) await db.exec(fs.readFileSync('supabase/migrations/' + file, 'utf8'));
await db.query('insert into auth.users(id,email,raw_app_meta_data) select unnest($1::uuid[]),unnest($2::text[]),$3::jsonb', [[admin, hr, oldAccount, newAccount, spare], ['admin@gmail.com', 'hr@gmail.com', 'old@gmail.com', 'new@gmail.com', 'spare@gmail.com'], JSON.stringify({ nexerp_provisioned: true })]);
...
await login();
for (const [kind, code, name] of [['department', 'D1', '지원팀'], ['grade', 'STAFF', '사원'], ['grade', 'SENIOR', '대리'], ['position', 'MEMBER', '팀원'], ['position', 'TEAM_LEAD', '팀장']])
...
const rehire = (employee, revision, hireDate, accountMode = 'keep', profileId = null) => rpc('hr_create_reemployment', [company, employee, revision, JSON.stringify({ hireDate, siteId: null, department: 'D1', grade: 'STAFF', position: 'TEAM_LEAD', accountMode, profileId }), '재입사 승인']);
