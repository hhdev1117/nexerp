...
const fails = async (fn, code = '42501', message) => {
    await assert.rejects(fn, (error) => error.code === code && (!message || error.message === message));
    checks++;
};
...
for (const file of fs
    ...
    .filter((file) => /^\d.*\.sql$/.test(file) && file < '20260915000900')
    .sort()) {
...
const employee = await rpc('hr_create_employee', [
    company,
    JSON.stringify({
        employeeNo: 'E1',
        name: '직원',
        profileId: employeeAccount,
        hireDate: '2020-01-01',
        siteId: mainSite,
        department: 'HQ',
        grade: 'STAFF',
...
const endSecondary = (targetEmployee, assignment, employeeRevision, assignmentRevision, endDate, reason = '겸직 종료') =>
    rpc('hr_end_secondary_assignment', [company, targetEmployee, assignment, employeeRevision, assignmentRevision, endDate, reason]);
