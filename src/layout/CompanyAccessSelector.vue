...
watch(() => [auth.user.value?.id, auth.profile.value?.is_active], ([id, active]) => {
    if (!id || !active) { runtime.reset(); return; }
    if (runtime.identityId.value !== id) { runtime.reset(); runtime.refresh(id); }
...
async function changeCompany(event) {
    ...
    if (!window.confirm('회사를 변경하면 현재 화면의 저장하지 않은 입력이 닫힙니다. 변경하시겠습니까?')) { event.target.value = runtime.context.value?.companyId || ''; return; }
...
<template>
    <section v-if="auth.profile.value?.is_active" class="company-access-bar" aria-label="현재 회사와 접근 권한">
        <label v-if="runtime.context.value?.companies.length" for="runtime-company">현재 회사<select id="runtime-company" :value="runtime.context.value.companyId || ''" :disabled="runtime.loading.value" @change="changeCompany"><option value="" disabled>회사 선택</option><option v-for="company in runtime.context.value.companies" :key="company.id" :value="company.id">{{ company.name }}</option></select></label>
...
<style scoped>
.company-access-bar { display: flex; gap: 1rem; align-items: center; flex-wrap: wrap; padding: .75rem 1rem; margin-bottom: 1rem; background: var(--surface-card); border: 1px solid var(--surface-border); border-radius: 8px; font-size: .9rem; }
label { display: flex; align-items: center; gap: .75rem; max-width: 100%; }
select,button { font: inherit; color: var(--text-color); background: var(--surface-card); border: 1px solid var(--surface-border); border-radius: 6px; padding: .6rem .75rem; min-height: 44px; max-width: 100%; }
button { cursor: pointer; margin-left: auto; } button:disabled { opacity: .6; cursor: wait; }
:focus-visible { outline: 3px solid var(--primary-color); outline-offset: 2px; }
@media(max-width:575px) { label { flex-direction: column; align-items: stretch; width: 100%; } button { margin-left: 0; } }
</style>
