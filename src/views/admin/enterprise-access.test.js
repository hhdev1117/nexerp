...
describe('enterprise access management', () => {
    ...
    it('saves a reviewed policy with a reason and the loaded revision', async () => {
        ...
        await wrapper.findAll('button').find((button) => button.text() === '변경 내용 확인').trigger('click');
        await flushPromises();
        expect(repo.save).not.toHaveBeenCalled();
        await wrapper.get('#change-reason').setValue('회사 권한 체계 초기 등록');
        await wrapper.findAll('button').find((button) => button.text() === '검토한 설정 저장').trigger('click');
        await flushPromises();
