    ...
    <main class="access-page">
        ...
        <section v-if="policy && !loading" data-testid="policy-editor">
            ...
            <fieldset :disabled="saving" class="access-editor">
                ...
                <section v-show="tab === 2" id="access-panel-2" role="tabpanel" aria-labelledby="access-tab-2">
                    <p v-if="accountsError" role="alert">{{ accountsError }} <button type="button" @click="loadAccounts">사용자 다시 불러오기</button></p>
                ...
                <section v-show="tab === 3" id="access-panel-3" role="tabpanel" aria-labelledby="access-tab-3">
                    <h2>최종 권한 확인</h2>
                    <p class="access-help">
                        편집 중인 정책에서 선택한 사용자의 자료 접근을 계산합니다. 계정 활성·AAL2·모듈 사용을 가정한 미리보기입니다. 연결된 직원의 현재 인사발령, 문서별 배정·하위 조직·결재 상태는 포함하지 않습니다. 실제 접근은 서버에서 현재 인사
                        원장을 반영해 판정합니다.
                    </p>
                    ...
                    <div class="access-record">
                        <label
                            >확인할 행동<select v-model="previewAction">
                                <option v-for="(label, key) in previewActions" :key="key" :value="key">{{ label }}</option>
                            </select></label
                        ><label
                            >자료 대상<select v-model="targetId">
                                <option value="">선택 사용자 본인</option>
                                <option v-for="member in policy.members" :key="member.id" :value="member.id">{{ member.name }}</option>
                            </select></label
                        >
                    </div>
