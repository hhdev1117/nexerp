    ...
    <div>
        ...
        <fieldset v-for="(member, index) in policy.members" :key="index" class="access-person">
            ...
            <div class="access-record">
                <label
                    >사용자 계정<select :value="member.id" :disabled="linked(member)" @change="selectAccount(member, $event.target.value)">
                        <option value="">계정 선택</option>
                        <option v-if="member.id && !accounts.some((item) => item.id === member.id)" :value="member.id">{{ member.name }} (계정 확인 필요)</option>
                        <option v-for="account in accounts" :key="account.id" :value="account.id" :disabled="policy.members.some((item) => item !== member && item.id === account.id)">
                            {{ account.displayName || account.email }} · {{ account.email }}
                        </option>
                    </select></label
                ><label>이름<input v-model.trim="member.name" maxlength="100" /></label><label>직급 코드<input v-model.trim="member.grade" /></label><label>직책 코드<input v-model.trim="member.position" /></label>
                <label
                ...
                >
                <label>조직 코드<input v-model.trim="member.organizationId" /></label
                ><label
                    >사업장<select v-model="member.siteId">
                        <option value="">지정 안 함</option>
                        <option v-for="site in sites" :key="site.id" :value="site.id">{{ site.name }}</option>
                    </select></label
                ><label>시작일<input v-model="member.from" type="date" /></label><label>종료일 (당일 제외)<input v-model="member.to" type="date" /></label><label class="access-check"><input v-model="member.active" type="checkbox" />활성 연결</label
