    ...
    <div class="access-grid">
        ...
        <div v-for="resource in visible" :key="resource.key" class="permission-row">
            ...
            <details class="scope-picker">
                <summary :aria-label="`${resource.label} ${ACTIONS[action]} 범위 편집`">
                    {{
                ...
                <label v-for="(label, key) in SCOPES" :key="key" class="access-check"
                    ><input type="checkbox" :data-scope="key" :aria-label="`${resource.label} ${ACTIONS[action]} ${label}`" :checked="scopesFor(resource.key).includes(key)" @change="changeScope(resource.key, key, $event.target.checked)" />{{
                        label
                    }}</label
                >
            </details>
