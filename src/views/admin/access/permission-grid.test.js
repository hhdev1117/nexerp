...
it('retains independent scopes when another scope is removed', async () => {
    const wrapper = mount(AccessPermissionGrid, { props: { resources: [{ key: 'dashboard', label: '대시보드' }], modelValue: [{ resource: 'dashboard', action: 'read', scope: 'self' }, { resource: 'dashboard', action: 'read', scope: 'assigned' }] } });
