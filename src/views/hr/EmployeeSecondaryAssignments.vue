...
<template>
    <section class="secondary" aria-labelledby="secondary-title">
        <header>
            <div>
                <h3 id="secondary-title">겸직</h3>
                <p>개인 직급은 주 소속의 직급을 그대로 사용합니다.</p>
            </div>
            <button v-if="history?.permissions.create" data-testid="secondary-create-open" @click="openCreate">겸직 등록</button>
        </header>
        <p v-if="loading" role="status">겸직 이력을 불러오는 중입니다.</p>
        ...
        <div v-for="row in history?.assignments" :key="row.id" class="assignment" :data-testid="`secondary-assignment-${row.status}`">
            ...
            <span>{{ row.position }} · 개인 직급 {{ row.grade }} · {{ statusLabel(row.status) }}</span
            ><span>{{ row.startDate }} ~ {{ row.endDate || '계속' }}</span> <button v-if="row.status === 'planned' && history.permissions.cancel" :disabled="saving" @click="openAction(row, 'cancel')">예정 취소</button
            ><button v-if="row.status === 'active' && history.permissions.end" :disabled="saving" @click="openAction(row, 'end')">겸직 종료</button>
        </div>
        <form v-if="mode === 'create'" @submit.prevent="create">
            <label for="secondary-site">사업장</label
            ><select id="secondary-site" v-model="draft.siteId">
                <option value="">선택</option>
                <option v-for="x in preparation.sites" :key="x.id" :value="x.id">{{ x.name }}</option>
            </select>
            <label for="secondary-department">부서</label
            ><select id="secondary-department" v-model="draft.department">
                <option value="">선택</option>
                <option v-for="x in preparation.references.filter((x) => x.kind === 'department')" :key="x.code" :value="x.code">{{ x.name }}</option>
            </select>
            <label for="secondary-position">직책</label
            ><select id="secondary-position" v-model="draft.position">
                <option value="">선택</option>
                <option v-for="x in preparation.references.filter((x) => x.kind === 'position')" :key="x.code" :value="x.code">{{ x.name }}</option>
            </select>
            <label for="secondary-start-date">시작일</label><input id="secondary-start-date" v-model="draft.startDate" type="date" /><label for="secondary-end-date">종료일(선택)</label
            ><input id="secondary-end-date" v-model="draft.endDate" type="date" /> <label for="secondary-reason">등록 사유</label><textarea id="secondary-reason" v-model="draft.reason" maxlength="2000" />
        ...
        <div v-if="mode === 'review'" data-testid="secondary-review-panel" class="review">
            <strong>등록 내용 확인</strong>
            <p>개인 직급 {{ referenceName('grade', preparation.primary.grade) }} 유지 · 겸직 직책 레벨 {{ positionLevel(draft.position) || '미매핑' }} · {{ referenceName('department', draft.department) }} 및 하위 조직</p>
            <p>{{ draft.startDate }} ~ {{ draft.endDate || '계속' }}</p>
            <button @click="mode = 'create'">수정</button><button data-testid="secondary-create-save" :disabled="saving" @click="create">{{ saving ? '저장 중' : '확정 등록' }}</button>
        </div>
        <form v-if="mode === 'end' || mode === 'cancel'" @submit.prevent="saveAction">
            <template v-if="mode === 'end'"><label for="secondary-action-end">종료일</label><input id="secondary-action-end" v-model="actionEndDate" type="date" /></template
            ><label for="secondary-action-reason">{{ mode === 'end' ? '종료' : '취소' }} 사유</label><textarea id="secondary-action-reason" v-model="actionReason" maxlength="2000" />
            <div>
                <button type="button" @click="mode = ''">닫기</button><button type="submit" data-testid="secondary-action-save" :disabled="saving" @click.prevent="saveAction">{{ saving ? '저장 중' : '확정' }}</button>
            </div>
        </form>
    </section>
</template>
