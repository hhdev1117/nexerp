...
const setup = () => mount(EmployeeSecondaryAssignments, { props: { companyId: 'company', employeeId: 'employee' } });
...
it('reviews and creates a secondary assignment with its capped scope', async () => {
    ...
    await wrapper.get('#secondary-site').setValue('site');
...
it('ends active and cancels planned assignments through explicit forms', async () => {
    const wrapper = setup();
    await flushPromises();
    const rows = wrapper.findAll('.assignment');
