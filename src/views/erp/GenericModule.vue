<script setup>
import { getModuleDefinition, statusSeverity } from '@/data/erp';
...
const { getGenericRecords, addGenericRecord } = useErpStore();
const keyword = ref('');
...
const emptyRecord = () => ({ subject: '', owner: '김서준', status: '진행중' });
...
const defaultStatuses = ['진행중', '승인 대기', '완료', '보류'];
...
const moduleRows = computed(() => {
    ...
    const baseRows = Array.isArray(route.meta.rows)
        ? route.meta.rows
        : [
              { id: `${prefix}-260911-04`, subject: `${moduleDefinition.value.title} 정기 업무`, owner: '김서준', updatedAt: '2026-09-11 11:42', status: '진행중' },
              { id: `${prefix}-260911-03`, subject: `${moduleDefinition.value.title} 신규 요청`, owner: '박지민', updatedAt: '2026-09-11 10:18', status: '승인 대기' },
              { id: `${prefix}-260910-12`, subject: `${moduleDefinition.value.title} 월간 마감`, owner: '이현우', updatedAt: '2026-09-10 16:35', status: '완료' },
              { id: `${prefix}-260909-08`, subject: `${moduleDefinition.value.title} 변경 검토`, owner: '최유진', updatedAt: '2026-09-09 14:07', status: '보류' },
              { id: `${prefix}-260908-02`, subject: `${moduleDefinition.value.title} 데이터 점검`, owner: '윤하늘', updatedAt: '2026-09-08 09:24', status: '완료' }
          ];

    return [...createdRows.value, ...baseRows];
});

const statusOptions = computed(() => [...new Set(moduleRows.value.map((row) => row.status || defaultStatuses[0]))]);
...
const filteredRows = computed(() => {
    ...
    return moduleRows.value.filter((row) => {
        ...
        const matchesKeyword = !query || [row.id, row.subject, row.owner].filter(Boolean).some((value) => String(value).toLocaleLowerCase('ko-KR').includes(query));
...
async function saveRecord() {
    ...
    const serial = String(createdRows.value.length + 1).padStart(2, '0');

    addGenericRecord(route.path, {
        id: `${modulePrefix.value}-${date.replaceAll('-', '').slice(2)}-N${serial}`,
    ...
    <div>
        ...
        <div class="card">
            <div class="flex flex-col gap-3 mb-6 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex flex-col gap-3 sm:flex-row">
                    ...
                    <Select v-model="selectedStatus" :options="statusOptions" placeholder="전체 상태" aria-label="업무 상태 필터" showClear class="w-full sm:w-40" />
                </div>
        ...
        <Dialog v-model:visible="recordDialog" modal :header="`${moduleDefinition.title} 신규 등록`" :style="{ width: '32rem' }" :breakpoints="{ '640px': 'calc(100vw - 2rem)' }">
            <form class="flex flex-col gap-5" novalidate @submit.prevent="saveRecord">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Select inputId="generic-status" v-model="record.status" :options="defaultStatuses" aria-labelledby="generic-status-label" class="w-full" />
                </div>
                <div class="flex justify-end gap-2 pt-2">
                    <Button type="button" label="취소" severity="secondary" text @click="recordDialog = false" />
                    <Button type="submit" label="등록" icon="pi pi-check" />
                </div>
            </form>
        </Dialog>
        ...
        <Dialog v-model:visible="detailDialog" modal :header="`${moduleDefinition.title} 상세`" :style="{ width: '32rem' }" :breakpoints="{ '640px': 'calc(100vw - 2rem)' }">
            <dl v-if="selectedRow" class="grid grid-cols-[7rem_minmax(0,1fr)] gap-x-4 gap-y-4 m-0">
                ...
                <dd class="m-0"><Tag :value="selectedRow.status" :severity="statusSeverity(selectedRow.status)" /></dd>
            </dl>
            <template #footer>
