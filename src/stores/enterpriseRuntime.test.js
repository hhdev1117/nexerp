...
const ctx = (companyId = 'a') => ({ mode: 'active', companyId, companies: [{ id: 'a', name: 'A' }, { id: 'b', name: 'B' }], menuKeys: ['company'], companyActions: ['read'], siteActions: [{ id: 's', actions: ['read'] }], revision: 1 });
...
describe('enterprise runtime store', () => {
    ...
    it('ignores stale identity and reset responses', async () => {
        ...
        const a = store.refresh('u1'); const b = store.refresh('u2');
        second.resolve(ctx('b')); await b;
        first.resolve(ctx()); await a;
        ...
        const load = resetStore.refresh('u'); resetStore.reset(); late.resolve(ctx()); await load;
    ...
    it('rejects invalid selection and ignores stale company response', async () => {
        ...
        await store.refresh('u');
        const first = store.selectCompany('u', 'a'); const second = store.selectCompany('u', 'b');
        b.resolve(ctx('b')); await second; a.resolve(ctx()); await first;
        expect(store.context.value.companyId).toBe('b');
    ...
    it('rejects malformed repository context and never grants legacy menu access', async () => {
        ...
        await store.refresh('u'); expect(store.canAccess('company')).toBe(false);
        ...
        await legacy.refresh('u'); expect(legacy.context.value.mode).toBe('legacy'); expect(legacy.canAccess('company')).toBe(false);
    });
});
