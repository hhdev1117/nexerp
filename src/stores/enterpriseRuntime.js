...
export function createEnterpriseRuntimeStore({ repository = createEnterpriseRuntimeRepository() } = {}) {
    ...
    const reset = () => {
        sequence += 1; context.value = null; loading.value = false; error.value = null; identityId.value = null; availableCompanies = [];
    };
    const refresh = async (identity, companyId = null) => {
        ...
        context.value = null; error.value = null; loading.value = Boolean(identity);
        if (!identity) { availableCompanies = []; return null; }
        try {
            ...
            if (request !== sequence) return null;
            context.value = null; availableCompanies = []; error.value = '권한 정보를 불러오지 못했습니다. 다시 시도해 주세요.';
            return null;
        } finally {
    ...
    const selectCompany = async (identity, companyId) => {
        if (identity !== identityId.value || !availableCompanies.includes(companyId)) {
            reset(); identityId.value = identity || null; error.value = '접근할 수 있는 회사를 선택해 주세요.'; return null;
        }
        return refresh(identity, companyId);
    };
    const active = () => context.value?.mode === 'active' && !loading.value;
    return { context, loading, error, identityId, refresh, selectCompany, reset,
