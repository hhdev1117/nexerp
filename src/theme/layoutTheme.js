...
import { DEFAULT_UI_PREFERENCES, normalizeUiPreferences } from '@/domain/uiPreferences';

export const DEFAULT_LAYOUT_PREFERENCES = DEFAULT_UI_PREFERENCES;
export const normalizeLayoutPreferences = normalizeUiPreferences;

const presets = { Aura, Lara, Nora };

export const presetOptions = Object.freeze(Object.keys(presets));

export const primaryColors = Object.freeze([
    { name: 'noir', palette: {} },
...
function getPresetExtension(primaryName) {
    const color = primaryColors.find(({ name }) => name === primaryName) || primaryColors.find(({ name }) => name === DEFAULT_LAYOUT_PREFERENCES.primary);
    if (color.name === 'noir') {
        return {
            semantic: {
                primary: {
...
export function applyPrimeTheme(preferences) {
    const normalized = normalizeLayoutPreferences(preferences);
    const surfacePalette = surfaces.find(({ name }) => name === normalized.surface)?.palette;
    $t().preset(presets[normalized.preset]).preset(getPresetExtension(normalized.primary)).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
    return normalized;
}
