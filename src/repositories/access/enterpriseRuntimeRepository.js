...
export class EnterpriseRuntimeRepositoryError extends Error {
    constructor(code, message) { super(message); this.name = 'EnterpriseRuntimeRepositoryError'; this.code = code; }
}
...
const invalid = () => { throw new EnterpriseRuntimeRepositoryError('invalid_runtime_response', '권한 응답을 확인할 수 없습니다. 다시 시도해 주세요.'); };
...
export function validateEnterpriseRuntimeContext(row) {
    ...
    return { mode: row.mode, companyId: row.companyId, companies: row.companies.map(({ id, name }) => ({ id, name })), menuKeys: [...row.menuKeys], revision: row.revision, companyActions: [...companyActions], siteActions: siteActions.map(({ id, actions }) => ({ id, actions: [...actions] })) };
}
...
export function createEnterpriseRuntimeRepository(client = getSupabaseClient()) {
    ...
    return {
        ...
        listAdminSites: (companyId) => run('enterprise_access_sites', { target_company: companyId }, (rows) => companiesValid(rows) && rows.every((row) => row.companyId === companyId) ? rows.map(({ id, name, companyId: owner }) => ({ id, name, companyId: owner })) : invalid())
    };
}
