...
describe('Supabase master repository', () => {
    ...
    it.each([
        ['companies_business_number_key', 'duplicate_business_number'],
        ['companies_code_key', 'duplicate_code']
    ])('maps company constraint %s to %s', async (constraint, expected) => {
        ...
        await expect(repository.createCompany({ code: 'NXT', name: '넥서스', businessNumber: '1208812345' })).rejects.toMatchObject({ code: expected });
    });
    ...
    it('updates only supplied mutable partner fields and preserves omitted values and the immutable code', async () => {
        const fixture = makeClient({ single: { data: { ...partnerRow, name: '서울 제일유통', is_vendor: true }, error: null } });
        const repository = createSupabaseMasterRepository(fixture.client);

        await expect(repository.updatePartner('p-1', { code: 'CHANGED', name: '서울 제일유통', isVendor: true, businessNumber: undefined, id: 'ignored' })).resolves.toEqual({
            ...partner,
            name: '서울 제일유통',
            isVendor: true
        });
    ...
    it.each([
        ...
        [{ code: '23502', message: 'null value in column "name" violates not-null constraint' }, 'invalid_value'],
        [{ code: '22P02', message: 'invalid input syntax for type uuid' }, 'invalid_value'],
        [{ code: '23503', message: 'insert or update violates foreign key constraint' }, 'not_found'],
