...
const action = (row) =>
    row &&
    uuid(row.id) &&
    ['transfer', 'terminate'].includes(row.type) &&
    date(row.effectiveDate) &&
    nullableUuid(row.siteId) &&
    ['department', 'grade', 'position', 'reason'].every((key) => text(row[key])) &&
...
const mapping = (row) =>
    row &&
    ['grade', 'position'].includes(row.kind) &&
    text(row.code) &&
    [1, 2, 3, 4, 5].includes(row.level) &&
    nullableDate(row.from) &&
    nullableDate(row.to);
...
export function createHrEmploymentRepository(client = getSupabaseClient()) {
        ...
        try {
            ...
            const code =
                cause?.code === '40001'
                    ...
                    : cause?.code === '42501'
                      ? 'access_denied'
                      : cause?.code === '22023' && trusted.includes(cause.message)
                        ? cause.message
                        : 'hr_employment_request_failed';
    ...
    return {
        loadHistory: (companyId, employeeId) =>
            request('hr_employment_history', { target_company: companyId, target_employee: employeeId }, (data) => historyValid(data, companyId, employeeId)),
        prepareRehire: (companyId, employeeId) =>
            request('hr_prepare_rehire', { target_company: companyId, target_employee: employeeId }, (data) => preparationValid(data, companyId, employeeId)),
        createReemployment: (companyId, employeeId, revision, document, reason) =>
            request(
                'hr_create_reemployment',
                { target_company: companyId, target_employee: employeeId, expected_revision: revision, employment_document: document, change_reason: reason },
                (data) => historyValid(data, companyId, employeeId)
            ),
        cancelPlanned: (companyId, employeeId, employmentId, revision, reason) =>
            request(
                'hr_cancel_planned_employment',
                { target_company: companyId, target_employee: employeeId, target_employment: employmentId, expected_revision: revision, change_reason: reason },
                (data) => historyValid(data, companyId, employeeId)
            )
    };
}
