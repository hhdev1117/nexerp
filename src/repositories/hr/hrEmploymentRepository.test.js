...
describe('HR employment repository', () => {
    it('uses exact history, preparation, create and cancellation contracts', async () => {
        const rpc = vi.fn().mockResolvedValueOnce({ data: history() }).mockResolvedValueOnce({ data: preparation() }).mockResolvedValue({ data: history() });
    ...
    it('rejects malformed histories and duplicate cycle identities', async () => {
        const bad = [null, { ...history(), companyId: 'foreign' }, { ...history(), employeeRevision: 0 }, { ...history(), permissions: { create: true } }, { ...history(), employments: [history().employments[0], history().employments[0]] }];
        for (const patch of [
            { sequenceNo: 0 },
            { hireDate: 'bad' },
            { endDate: 'bad' },
