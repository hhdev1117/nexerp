...
const entry = (row) =>
    row && uuid(row.id) && nullableUuid(row.beforeAccountId) && nullableUuid(row.afterAccountId) && text(row.beforeAccountName) && text(row.afterAccountName) && text(row.reason) && text(row.createdAt) && Number.isFinite(Date.parse(row.createdAt));
...
export function createHrAccountRepository(client = getSupabaseClient()) {
    ...
    return {
        ...
        linkAccount: (companyId, employeeId, revision, profileId, reason) =>
            request('hr_link_employee_account', { target_company: companyId, target_employee: employeeId, expected_revision: revision, target_profile: profileId, change_reason: reason }, (data) => optionsValid(data, companyId, employeeId))
    };
}
